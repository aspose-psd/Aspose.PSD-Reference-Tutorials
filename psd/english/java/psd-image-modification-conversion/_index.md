---
title: Convert PSD Layers to PNG using Aspose.PSD for Java – Image Modification & Conversion
...
Below is a simple Java example that demonstrates how to load a PSD file and export each layer as a PNG image:

```java
// Load PSD and export each layer as PNG
PsDocument psd = new PsDocument("input.psd");
for (int i = 0; i < psd.getLayers().size(); i++) {
    Layer layer = psd.getLayers().get(i);
    String outPath = "layer_" + i + ".png";
    layer.save(outPath, new PngOptions());
}
```
