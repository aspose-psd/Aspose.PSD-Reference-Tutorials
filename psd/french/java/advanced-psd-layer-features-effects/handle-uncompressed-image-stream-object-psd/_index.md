...
- **Quelle bibliothèque gérer les flux non compressés ?** Aspose.PSD pour Java fournit une prise en charge complète des données d'image brutes (non compressées).
...
## Comment exporter du PSD vers PNG en utilisant Aspose.PSD pour Java
...
## Prérequis
Avant de Sous-marin dans le code, contrôlez‑nous que vous avez tout ce qu'il faut pour commencer ce voyage. Voici les prérequis :
...
Vous devez télécharger et installer la bibliothèque Aspose.PSD. Cette bibliothèque puissante vous permet de manipuler facilement les fichiers PSD. Vous pouvez obtenir la dernière version via [ce lien](https://releases.aspose.com/psd/java/).
...
Pour démarrer, nous devons importer les packages nécessaires pour travailler avec Aspose.PSD. Vous trouverez ci-dessous les importations généralement requises pour la manipulation de fichiers PSD.
...
## Étape 2 : Créer un flux de sortie de type tableau d’octets
...
## Manipuler les calques PSD avec un objet graphique
...
- **L’objet Graphics apparaît vide** – assurez-vous de dessiner sur la bonne instance `PsdImage` (utilisez celle que vous avez chargée, pas celle créée, sauf si c'est intentionnel).
...
**Q : Puis‑je utiliser l’objet graphique pour éditer uniquement un calque spécifique ?**
...
R : Bien sûr. Utilisez le supplément approprié de `Image.save` avec `PngOptions` après l'édition— c'est la méthode standard pour **exporter PSD en PNG**.

**Q : Quelle version de Java est requise ?**
R : Aspose.PSD pour Java prend en charge JDK8 et les versions ultérieures.
